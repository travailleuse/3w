\section{代数}
\subsection{十进制加减法}
任意的十进制数字可以写成\(\dis\sum_{n\in\Zs}^{}a_n10^n\)，其中\(a_n\in \{0\le a_n\l\le10 \left| a_n\in\Zs\right.\}\)
...
设\(d_i = s - 10\)，代入\(10d_i = 10^ns- 10^{n+1}\)。这解释了减法借十的原因。

\subsection{基本不等式}
我们都知道基本不等式\(\dis \frac{a+b}{2}\ge\sqrt{ab},a\ge0, b\ge0\)，当且仅当\(a=b\)时区等号。
\begin{proof}\itshape
\[
...
如上图所示，\(\odot O, AB\perp PT, AT\cdot TB = (OP+OT)(OP-OA) = OP^2-OA^2 = PT^2\)，即
...
\begin{itemize}
    ...
    \item 存在单位元，即幺元:\(e\in S\).对与\(\forall a\in S\)，都有\(e \circ a = a\)；
...
下面证明群的单位元始唯一的，即\(e_l = e_r\)
\begin{proof}
    \begin{align*}
        e_l = e_le_re_r = e_re_r = e_r
    \end{align*}
\end{proof}
...
\begin{proof}
    ...
    \[
    a\circ(a^{-1}\circ a) = (a\circ a^{-1})\circ a= e
    \]\
   \hfill\textbf{证完}
...
所有的复数（\(\Cs\)）都可以表示成\(z=a+b\mathrm{i}\)的形式，其中\(a\)为为复数\(z\)的实部，
...
复数的加减运算
\begin{itemize}
    \item \(z_1 + z_2 = (a+b\mathrm{i}) + (c + d\mathrm{i}) = (a+b)+(c+d)\mathrm{i}\)
    \item \(z_1 - z_2 = (a+b\mathrm{i}) - (c + d\mathrm{i}) = (a-b)+(c-d)\mathrm{i}\)
\end{itemize}
...
\begin{itemize}
    ...
    \item \(\dis \frac{z_1}{z_2} = \frac{r_1}{r_1}(\cos(\theta_1-\theta_2) + \mathrm{i}\sin(\theta_1-\theta_2))\)
\end{itemize}
